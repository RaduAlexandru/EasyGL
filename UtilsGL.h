...
inline int gl_internal_format2cv_type(const GLint internal_format){
    ...
    switch ( internal_format ) {
       ...
       default:  LOG(FATAL) << "Internal format "<< std::hex << internal_format << std::dec <<  " unkown. We support only 8UI and 32F and 1,3 and 4 channels"; break;
    }
...
inline void cv_type2gl_formats(GLint& internal_format, GLenum& format, GLenum& type, const int cv_type, const bool store_as_normalized_vals){
    ...
    if(depth==CV_8U && store_as_normalized_vals){
        ...
        switch ( channels ) {
           ...
           default:  LOG(FATAL) << "Nr of channels not supported. We only support 1,3 and 4"; break;
        }
